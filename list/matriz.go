...
func (m *matriz) noExisten(producto *Product, x int, y int) {
	...
	v := m.lst_v.search(y) //vamos a buscar el nodo que acabamos de insertar para poder enlazarlo
...
func (m *matriz) existeVertical(producto *Product, x int, y int) {
	...
	v := m.lst_v.search(y)              //BUSCO EL NODO EN LA LISTA VERTICAL
	nuevo := nodoMatriz(x, y, producto) //creamos nuevo nodo tipo matriz
	agregado := false
	aux := v.derecha
...
func (m *matriz) existeHorizontal(producto *Product, x int, y int) {
	m.lst_v.insert(y)                   //insertamos en la lista que emula la cabecera horizontal
	h := m.lst_h.search(x)              //BUSCO EL NODO EN LA LISTA HORIZONTAL
	v := m.lst_v.search(y)              //BUSCO EL NODO EN LA LISTA VERTICAL
	nuevo := nodoMatriz(x, y, producto) //creamos nuevo nodo tipo matriz
	agregado := false
